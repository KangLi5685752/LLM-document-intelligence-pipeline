"""Offline tests for the bounded Stage 4D v0.3 OpenAI development transaction."""
...
from document_intelligence.llm_extraction import (
    openai_development_execution_v0_3 as execution,
)
...
from document_intelligence.llm_extraction import (
    openai_development_execution as v0_1_execution,
)
from document_intelligence.llm_extraction import (
    openai_development_execution_v0_2 as v0_2_execution,
)
...
from document_intelligence.llm_extraction.openai_development_execution_v0_3 import (
...
def _frozen_models() -> tuple[
    ...
    return (
        OpenAIDevelopmentExecutionPlanV03.model_validate_json(
            PLAN_SOURCE.read_bytes()
        ),
        OpenAIDevelopmentManifestV03.model_validate_json(
...
def _authorization(**updates: Any) -> OpenAIDevelopmentExecutionAuthorizationV03:
    values = {
    ...
    provisional = OpenAIDevelopmentExecutionAuthorizationV03.model_construct(
        **values,
        authorization_sha256="0" * 64,
    )
    return OpenAIDevelopmentExecutionAuthorizationV03.model_validate(
        {
            **values,
...
def _inputs(
    tmp_path: Path,
    *,
    authorization: OpenAIDevelopmentExecutionAuthorizationV03 | None = None,
    ...
    authorization_path.write_bytes(
        development_authorization_bytes_v0_3(authorization or _authorization())
    )
...
def _prepared_invocations(
    ...
    for plan_item, manifest_item in zip(
        ...
        request = LLMExtractionRequestV03.model_construct(
            ...
            provider_configuration_id=execution.OPENAI_PROVIDER_CONFIGURATION_ID_V0_3,
            model_configuration_id=execution.OPENAI_MODEL_CONFIGURATION_ID_V0_3,
            output_contract_id="candidate-extraction-result-0.1",
            evidence_blocks=(block,),
        )
        identity = CacheIdentityV03.from_request(request)
...
def _readiness(tmp_path: Path):
    ...
    readiness = execution._validate_openai_development_execution_readiness_v0_3(
...
def _execute(
    ...
    *,
    ...
    return execution._execute_openai_development_transaction_v0_3(
        authorization_path=paths[0],
        pricing_path=paths[1],
        data_controls_path=paths[2],
        repository_root=root,
...
def test_readiness_loads_exact_frozen_plan_manifest_without_side_effects(
    ...
    assert PLAN_SOURCE.stat().st_size == execution.EXECUTION_PLAN_ARTIFACT_BYTES
    assert uppercase_sha256_bytes(PLAN_SOURCE.read_bytes()) == (
        execution.EXECUTION_PLAN_OUTER_SHA256
    )
    assert MANIFEST_SOURCE.stat().st_size == execution.MANIFEST_ARTIFACT_BYTES
    assert uppercase_sha256_bytes(MANIFEST_SOURCE.read_bytes()) == (
        execution.MANIFEST_CANONICAL_LF_SHA256
    )
    assert readiness.plan.provider_binding.provider_configuration_id == (
        execution.OPENAI_PROVIDER_CONFIGURATION_ID_V0_3
    )
    assert readiness.plan.provider_binding.model_configuration_id == (
        execution.OPENAI_MODEL_CONFIGURATION_ID_V0_3
    )
    assert readiness.plan.provider_binding.response_schema_name == (
        execution.OPENAI_RESPONSE_SCHEMA_NAME_V0_3
    )
    assert readiness.plan.provider_binding.strict_schema_sha256 == (
        "C39E96B77BC2E9BEC3DF191071BC0C8B8F1AE545228A7D6CB6DF0CCA44E8269E"
    )
    assert [item.request_id for item in readiness.plan.invocations] == [
    ...
    assert readiness.plan.invocations[-1].repeated_primary_request_id == (
        "llm-v0.3-S004-primary-001"
    )
    assert len(
        {item.cache_identity_sha256 for item in readiness.plan.invocations}
    ) == 8
...
def test_readiness_rejects_frozen_artifact_tampering(
    ...
    with pytest.raises(Stage4BError) as captured:
        execution._validate_openai_development_execution_readiness_v0_3(
            authorization_path=paths[0],
            pricing_path=paths[1],
            data_controls_path=paths[2],
            repository_root=root,
...
@pytest.mark.parametrize(
    ...
    ids=("previous_utc_date", "future_timestamp"),
)
def test_stale_or_postdated_authorization_fails_before_reconstruction(
    tmp_path: Path,
    ...
    paths = _inputs(
        tmp_path,
        authorization=_authorization(authorized_at_utc=authorized_at),
    )
    reconstruction_calls = 0

    def forbidden_reconstructor(*_args: Any):
    ...
    with pytest.raises(Stage4BError) as captured:
        execution._validate_openai_development_execution_readiness_v0_3(
            ...
            repository_root=root,
            clock=lambda: NOW,
            reconstructor=forbidden_reconstructor,
        )
...
def test_previous_date_authorization_consumes_no_invocation_or_side_effect(
    tmp_path: Path,
) -> None:
    root = _repository(tmp_path)
    paths = _inputs(
        tmp_path,
        authorization=_authorization(authorized_at_utc=NOW - timedelta(days=1)),
    )
    counters = {"reconstructor": 0, "key": 0, "client": 0, "provider": 0}

    def forbidden(name: str):
        def call(*_args: Any, **_kwargs: Any):
            counters[name] += 1
            raise AssertionError
    ...
    with pytest.raises(Stage4BError) as captured:
        execution._execute_openai_development_transaction_v0_3(
            authorization_path=paths[0],
            pricing_path=paths[1],
            data_controls_path=paths[2],
            repository_root=root,
            ...
            confirmation=EXECUTION_CONFIRMATION,
            clock=lambda: NOW,
            api_key_reader=forbidden("key"),
            client_factory=forbidden("client"),
            reconstructor=forbidden("reconstructor"),
            provider_observation=forbidden("provider"),
            local_validator=_candidate_output,
        )
    ...
    assert counters == {"reconstructor": 0, "key": 0, "client": 0, "provider": 0}
    plan, _ = _frozen_models()
    assert not root.joinpath(*Path(plan.cache_policy.relative_cache_root).parts).exists()
    assert not root.joinpath(*Path(execution.ATTEMPT_MARKER_ROOT).parts).exists()
    assert not root.joinpath(*Path(execution.FAILURE_RECORD_ROOT).parts).exists()
    assert not root.joinpath(*Path(execution.EXECUTION_RECORD_PATH).parts).exists()


def test_same_day_earlier_authorization_remains_valid(tmp_path: Path) -> None:
    root = _repository(tmp_path)
    ...
    assert readiness.inputs.authorization.authorized_at_utc == authorized_at
    assert readiness.existing_execution_record is None
...
def test_cache_miss_orders_marker_before_client_call_cache_and_parse(
    ...
    def provider(_client: object, request: LLMExtractionRequestV03):
        nonlocal provider_calls
        events.append("provider")
        provider_calls += 1
        return _observation(request)

    def local(request: LLMExtractionRequestV03, response: LLMProviderResponse):
        identity = CacheIdentityV03.from_request(request)
...
def test_attempt_marker_without_cache_permanently_blocks_retry(tmp_path: Path) -> None:
    ...
    execution._write_exclusive(
        marker_path, execution.attempt_marker_bytes_v0_3(marker), marker=True
    )
    calls = 0
    with pytest.raises(Stage4BError) as captured:
        _execute(
    ...
    assert calls == 0
    assert load_development_attempt_marker_v0_3(marker_path) == marker
...
def test_losing_exclusive_marker_install_creates_no_failure_record(
    ...
    pending_markers: list[
        execution.OpenAIDevelopmentInvocationAttemptMarkerV03
    ] = []
    counters = {"key": 0, "client": 0, "provider": 0}
    ...
    def racing_write_exclusive(path: Path, payload: bytes, *, marker: bool) -> None:
        if marker and path == marker_path:
            ...
            assert payload == execution.attempt_marker_bytes_v0_3(
                pending_markers[0]
            )
            original_write_exclusive(
                marker_path,
                execution.attempt_marker_bytes_v0_3(competing_marker),
                marker=True,
            )
    ...
    assert counters == {"key": 0, "client": 0, "provider": 0}
    assert load_development_attempt_marker_v0_3(marker_path) == competing_marker
...
def test_canonical_conflicting_marker_fails_before_provider_boundaries(
    ...
    provisional = execution.OpenAIDevelopmentInvocationAttemptMarkerV03.model_construct(
        **values,
        marker_sha256="0" * 64,
    )
    conflicting = (
        execution.OpenAIDevelopmentInvocationAttemptMarkerV03.model_validate(
            {
                **values,
    ...
    execution._write_exclusive(
        marker_path,
        execution.attempt_marker_bytes_v0_3(conflicting),
        marker=True,
    )
    counters = {"key": 0, "client": 0, "provider": 0}
...
def test_client_construction_failure_preserves_marker_and_creates_no_cache(
    ...
    assert load_development_attempt_marker_v0_3(marker_path)
    assert load_development_failure_record_v0_3(failure_path).failure_stage == (
        "client_construction"
    )
    with pytest.raises(Stage4BError) as miss:
        ResponseCache(readiness.cache_root).read(invocation.cache_identity)
...
def test_local_parse_failure_preserves_cache_and_reprocesses_without_calls(
    ...
    def fails_last(request: LLMExtractionRequestV03, response: LLMProviderResponse):
...
def test_first_local_semantic_failure_stops_before_invocation_two(
    tmp_path: Path,
) -> None:
    root, paths, readiness = _readiness(tmp_path)
    provider_calls = 0
    ...
    def invalid_local(*_args: object) -> ValidatedCandidateOutput:
    ...
    with pytest.raises(Stage4BError) as captured:
        _execute(
            ...
            paths,
            provider_observation=provider,
            local_validator=invalid_local,
        )

    first, second = readiness.invocations[:2]
    assert captured.value.code is Stage4BErrorCode.SCHEMA_INVALID
    assert provider_calls == 1
    assert local_calls == 1
    assert ResponseCache(readiness.cache_root).read(first.cache_identity)
    with pytest.raises(Stage4BError) as cache_miss:
        ResponseCache(readiness.cache_root).read(second.cache_identity)
    assert cache_miss.value.code is Stage4BErrorCode.CACHE_MISS
    first_marker = root.joinpath(
        *Path(first.plan.attempt_marker_relative_path).parts
    )
    first_failure = root.joinpath(
        *Path(first.plan.failure_record_relative_path).parts
    )
    second_marker = root.joinpath(
        *Path(second.plan.attempt_marker_relative_path).parts
    )
    assert first_marker.is_file()
    failure = load_development_failure_record_v0_3(first_failure)
    assert failure.failure_stage == "local_parse"
    assert failure.cache_present is True
    assert failure.cache_install_completed is True
    assert failure.provider_call_occurred is True
    assert failure.local_parse_started is True
...
def test_cache_recovery_preserves_exact_original_same_call_provenance(
    ...
    def observation(_client: object, request: LLMExtractionRequestV03):
        return _observation(
            request,
    ...
    def fail_last(request: LLMExtractionRequestV03, response: LLMProviderResponse):
        nonlocal parse_count
        parse_count += 1
        if parse_count == 8:
            raise Stage4BError(Stage4BErrorCode.SCHEMA_INVALID, "fictional")
...
def test_utc_date_rollover_before_first_miss_stops_before_marker_and_provider(
    ...
    next_day_paths = _inputs(
        tmp_path,
        authorization=_authorization(authorized_at_utc=reviewed_at),
    ...
    def recovery_provider(_client: object, request: LLMExtractionRequestV03):
...
def test_utc_date_rollover_between_misses_stops_next_invocation_before_marker(
    ...
    with pytest.raises(Stage4BError) as recovery_error:
        _execute(
            root,
            next_day_paths,
            clock=lambda: reviewed_at,
    ...
    assert recovery_error.value.code is (
        Stage4BErrorCode.DEVELOPMENT_ATTEMPT_ALREADY_EXISTS
    )
    assert recovery_counters == {"key": 0, "client": 0, "provider": 0}
    assert not root.joinpath(
        *Path(plan.invocations[1].attempt_marker_relative_path).parts
    ).exists()
    assert not second_failure_path.exists()
...
def test_failure_record_and_raised_error_scrub_credential_fragments(
    ...
    def failure(_client: object, _request: LLMExtractionRequestV03):
    ...
    record = load_development_failure_record_v0_3(failure_path)
    with pytest.raises(ValidationError):
        v0_1_execution.OpenAIDevelopmentInvocationFailureRecordV01.model_validate(
...
def test_eight_valid_invocations_install_one_canonical_final_record(
    ...
    assert raw == execution_record_bytes_v0_3(record)
    assert raw.endswith(b"\n") and not raw.endswith(b"\n\n")
    assert load_development_execution_record_v0_3(result.execution_record_path) == record
    with pytest.raises(ValidationError):
        v0_1_execution.OpenAIDevelopmentExecutionRecordV01.model_validate(
            record.model_dump(mode="python")
        )
    first_marker_path = root.joinpath(
        *Path(result.readiness.plan.invocations[0].attempt_marker_relative_path).parts
    )
    first_marker = load_development_attempt_marker_v0_3(first_marker_path)
    with pytest.raises(ValidationError):
        v0_1_execution.OpenAIDevelopmentInvocationAttemptMarkerV01.model_validate(
            first_marker.model_dump(mode="python")
        )
...
def test_recomputed_final_hash_cannot_hide_contradictory_version_provenance(
    ...
    with pytest.raises(Stage4BError) as captured:
        load_development_execution_record_v0_3(result.execution_record_path)
...
def test_public_execution_api_exposes_no_root_provider_or_cache_bypass() -> None:
    ...
    assert tuple(
        inspect.signature(execution.execute_openai_development_v0_3).parameters
    ) == (
        "authorization_path",
        "pricing_path",
        "data_controls_path",
...
def test_production_provider_observation_annotations_resolve() -> None:
    hints = get_type_hints(execution._production_provider_observation)

    assert hints["request"] is LLMExtractionRequestV03


def test_v0_1_v0_2_and_v0_3_authorizations_cannot_cross() -> None:
    v0_3 = _authorization()
    with pytest.raises(ValidationError):
        v0_1_execution.OpenAIDevelopmentExecutionAuthorizationV01.model_validate(
            v0_3.model_dump(mode="python")
        )

    values = {
        **v0_3.model_dump(mode="python", exclude={"authorization_sha256"}),
        "execution_id": v0_1_execution.EXECUTION_ID,
        "authorization_scope": v0_1_execution.AUTHORIZATION_SCOPE,
        "execution_plan_sha256": (
            v0_1_execution.EXPECTED_EXECUTION_PLAN_SHA256
        ),
        "manifest_sha256": v0_1_execution.MANIFEST_SELF_SHA256,
    }
    provisional = (
        v0_1_execution.OpenAIDevelopmentExecutionAuthorizationV01.model_construct(
    ...
    v0_1 = (
        v0_1_execution.OpenAIDevelopmentExecutionAuthorizationV01.model_validate(
            {
                **values,
                "authorization_sha256": v0_1_execution._canonical_hash(
                    provisional, "authorization_sha256"
                ),
            }
        )
    ...
    with pytest.raises(ValidationError):
        OpenAIDevelopmentExecutionAuthorizationV03.model_validate(
            v0_1.model_dump(mode="python")
        )

    v0_2_values = {
        **v0_3.model_dump(mode="python", exclude={"authorization_sha256"}),
        "execution_id": v0_2_execution.EXECUTION_ID,
        "authorization_scope": v0_2_execution.AUTHORIZATION_SCOPE,
        "execution_plan_sha256": v0_2_execution.EXPECTED_EXECUTION_PLAN_SHA256,
        "manifest_sha256": v0_2_execution.MANIFEST_SELF_SHA256,
    }
    v0_2_provisional = (
        v0_2_execution.OpenAIDevelopmentExecutionAuthorizationV02.model_construct(
            **v0_2_values, authorization_sha256="0" * 64
        )
    )
    v0_2 = v0_2_execution.OpenAIDevelopmentExecutionAuthorizationV02.model_validate(
        {
            **v0_2_values,
            "authorization_sha256": v0_2_execution._canonical_hash(
                v0_2_provisional, "authorization_sha256"
            ),
        }
    )
    with pytest.raises(ValidationError):
...
def test_v0_1_v0_2_and_v0_3_cache_identities_cannot_cross(tmp_path: Path) -> None:
    _, _, readiness = _readiness(tmp_path)
    v0_3 = readiness.invocations[0].cache_identity
    with pytest.raises(ValidationError):
        CacheIdentity.model_validate(v0_3.model_dump(mode="python"))

    v0_1_values = {
        **v0_3.model_dump(mode="python"),
        "experiment_id": "llm-extraction-baseline-v0.1",
    }
    v0_1 = CacheIdentity.model_validate(v0_1_values)
    with pytest.raises(ValidationError):
        CacheIdentityV03.model_validate(v0_1.model_dump(mode="python"))

    v0_2_values = {
        **v0_3.model_dump(mode="python"),
        "experiment_id": "llm-extraction-baseline-v0.2",
    }
    v0_2 = CacheIdentityV02.model_validate(v0_2_values)
    with pytest.raises(ValidationError):
        CacheIdentityV03.model_validate(v0_2.model_dump(mode="python"))


def test_committed_v0_1_marker_and_failure_remain_valid_but_not_v0_3() -> None:
    v0_1_root = REPOSITORY_ROOT / (
        "reports/llm_extraction/openai_development_execution/"
        "openai-gpt-5.4-mini-five-source-development-v0.1"
    )
    marker_path = next((v0_1_root / "attempts").glob("*.attempt.json"))
    failure_path = next((v0_1_root / "failures").glob("*.failure.json"))

    marker = v0_1_execution.load_development_attempt_marker(marker_path)
    failure = v0_1_execution.load_development_failure_record(failure_path)
    assert marker.execution_id == v0_1_execution.EXECUTION_ID
    assert failure.execution_id == v0_1_execution.EXECUTION_ID
    with pytest.raises(ValidationError):
        execution.OpenAIDevelopmentInvocationAttemptMarkerV03.model_validate(
            marker.model_dump(mode="python")
        )
    with pytest.raises(ValidationError):
        execution.OpenAIDevelopmentInvocationFailureRecordV03.model_validate(
