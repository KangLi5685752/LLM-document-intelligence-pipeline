"""Offline default-deny CLI tests for the Stage 4D v0.3 development transaction."""
...
from document_intelligence.llm_extraction import (
    openai_development_execution_v0_3_cli as cli,
)
...
def test_cli_defaults_to_no_call_readiness(
    ...
    monkeypatch.setattr(
        execution, "_validate_openai_development_execution_readiness_v0_3", readiness
    )
    ...
    assert capsys.readouterr().out == (
        ...
        'execution-v0.3","execution_plan_sha256":"12191955D5ED1F6EBF0B0BC97AA6A2'
        'EF11B164186645FD68D6270D8A241A0F0A","mode":"readiness",'
        '"status":"ready"}\n'
    )


def test_cli_reports_existing_exact_record_as_already_complete(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        execution,
        "_validate_openai_development_execution_readiness_v0_3",
        lambda **_kwargs: SimpleNamespace(
            plan=SimpleNamespace(
                execution_id=execution.EXECUTION_ID,
                execution_plan_sha256=execution.EXPECTED_EXECUTION_PLAN_SHA256,
            ),
            existing_execution_record=object(),
        ),
    )
    ...
    assert capsys.readouterr().out.endswith(
        '"mode":"readiness","status":"already_complete"}\n'
    )
...
def test_cli_rejects_abbreviated_arguments_with_sanitized_output(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    with pytest.raises(SystemExit) as captured:
        cli._main_for_tests(
            ["--auth", str(tmp_path / "fictional.json")],
            repository_root=tmp_path,
        )
...
def test_public_entrypoints_expose_no_dependency_injection() -> None:
    ...
    assert tuple(inspect.signature(execution.execute_openai_development_v0_3).parameters) == (
        "authorization_path",
        "pricing_path",
        "data_controls_path",
...
def test_pyproject_exposes_default_deny_development_execution_cli() -> None:
    ...
    expected = (
        "run-openai-development-execution-v0-3 = "
        '"document_intelligence.llm_extraction.'
        'openai_development_execution_v0_3_cli:main"'
    )
