"""Offline contract tests for compatibility-first OpenAI preflight v0.3."""
...
def _authorization() -> OpenAIPreflightAuthorizationV03:
    return OpenAIPreflightAuthorizationV03(
...
def _observation(
    ...
    *,
    ...
    response = LLMProviderResponse(
        ...
        provider_request_id="req_fictional_v0_3",
        provider_response_id="resp_fictional_v0_3",
...
def _run(payload: object) -> tuple[OpenAIPreflightRecordV03, FakeProvider]:
...
def test_invalid_json_preserves_safe_post_response_metadata_without_raw_output() -> None:
    ...
    with pytest.raises(OpenAIPreflightPostResponseFailureV03) as captured:
        run_openai_synthetic_preflight(
            provider=provider,
            authorization=_authorization(),
            pricing_observation=_pricing(),
    ...
    assert metadata.provider_request_id == "req_fictional_v0_3"
    assert metadata.retry_count == 0
    serialized = canonical_json_bytes(metadata.model_dump(mode="json"))
    assert b"not-json" not in serialized


def test_v0_3_identities_and_payload_are_separate_and_safety_controls_are_unchanged() -> None:
    request = build_synthetic_openai_preflight_request()
    payload = build_openai_responses_payload(request)
    record, provider = _run(_base_payload())

    assert PREFLIGHT_ID == "openai-gpt-5.4-mini-synthetic-preflight-v0.3"
    assert PREFLIGHT_AUTHORIZATION_SCOPE == "single-synthetic-openai-preflight-v0.3"
    assert PREFLIGHT_ID != V0_2_ID
    assert PREFLIGHT_AUTHORIZATION_SCOPE != V0_2_SCOPE
    assert request.request_id == "synthetic-preflight-request-v0.3"
    assert request.evidence_blocks[0].evidence_id.endswith("v0.3")
    assert request.evidence_blocks[0].block_id.endswith("v0.3")
...
def test_v0_2_authorization_cannot_satisfy_v0_3_scope() -> None:
    ...
    with pytest.raises(ValidationError):
        OpenAIPreflightAuthorizationV03.model_validate(
            v0_2.model_dump(mode="python")
        )


def test_v0_3_contract_uses_no_environment_network_or_default_client(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def forbidden(*args: object, **kwargs: object) -> None:
        pytest.fail("v0.3 contract attempted environment, network or client access")
