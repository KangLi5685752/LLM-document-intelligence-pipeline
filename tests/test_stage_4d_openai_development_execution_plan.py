"""Tests for the Stage 4D bounded development execution plan."""
...
def _manifest() -> OpenAIDevelopmentManifestV01:
    return OpenAIDevelopmentManifestV01.model_validate_json(
        MANIFEST_PATH.read_bytes()
    )


def _plan() -> OpenAIDevelopmentExecutionPlanV01:
    return build_openai_development_execution_plan(_manifest())
...
def test_execution_plan_has_exact_readiness_identity() -> None:
    plan = _plan()
    raw = development_execution_plan_bytes(plan)
    ...
    assert OpenAIDevelopmentExecutionPlanV01.model_validate_json(raw) == plan
    assert hashlib.sha256(raw).hexdigest().upper() == (
        hashlib.sha256(development_execution_plan_bytes(plan))
        .hexdigest()
        .upper()
    )
...
def test_execution_plan_binds_exact_manifest_and_provider() -> None:
    ...
    assert plan.manifest_binding.artifact_bytes == 90809
...
def test_execution_plan_is_hash_only() -> None:
    payload = json.loads(development_execution_plan_bytes(_plan()))
