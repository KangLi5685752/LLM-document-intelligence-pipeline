"""Offline tests for the bounded Stage 4D v0.2 OpenAI development transaction."""
...
from document_intelligence.llm_extraction.openai_development_execution_v0_2 import (
...
def _frozen_models() -> tuple[
    ...
    return (
        OpenAIDevelopmentExecutionPlanV02.model_validate_json(
            PLAN_SOURCE.read_bytes()
        ),
        OpenAIDevelopmentManifestV02.model_validate_json(
...
def _authorization(**updates: Any) -> OpenAIDevelopmentExecutionAuthorizationV02:
    values = {
    ...
    provisional = OpenAIDevelopmentExecutionAuthorizationV02.model_construct(
        **values,
        authorization_sha256="0" * 64,
    )
    return OpenAIDevelopmentExecutionAuthorizationV02.model_validate(
        {
            **values,
...
def _inputs(
    tmp_path: Path,
    *,
    authorization: OpenAIDevelopmentExecutionAuthorizationV02 | None = None,
    ...
    authorization_path.write_bytes(
        development_authorization_bytes_v0_2(authorization or _authorization())
    )
...
def _prepared_invocations(
    ...
    for plan_item, manifest_item in zip(
        ...
        request = LLMExtractionRequestV02.model_construct(
        ...
        identity = CacheIdentityV02.from_request(request)
...
def _execute(
    ...
    *,
    ...
    return execution._execute_openai_development_transaction_v0_2(
        authorization_path=paths[0],
        pricing_path=paths[1],
        data_controls_path=paths[2],
        repository_root=root,
...
def test_production_reconstruction_reconciles_all_eight_frozen_requests() -> None:
    ...
    prepared = execution._reconstruct_invocations(
        REPOSITORY_ROOT, plan, manifest
    )

    assert tuple(item.plan for item in prepared) == plan.invocations
    assert tuple(item.manifest_identity for item in prepared) == (
        manifest.invocations
    )
    assert prepared[0].request.request_id == "llm-v0.2-S001-primary-001"
    assert prepared[-1].request.request_id == "llm-v0.2-S004-repeat-001"
    assert prepared[-1].manifest_identity.repeated_primary_request_id == (
        "llm-v0.2-S004-primary-001"
    )
    assert len({item.cache_identity_sha256 for item in plan.invocations}) == 8
...
def test_readiness_rejects_frozen_artifact_tampering(
    ...
    with pytest.raises(Stage4BError) as captured:
        execution._validate_openai_development_execution_readiness_v0_2(
            authorization_path=paths[0],
            pricing_path=paths[1],
            data_controls_path=paths[2],
            repository_root=root,
...
def test_missing_execution_gate_fails_before_key_client_or_artifact(
    ...
    for flag, phrase in ((False, None), (True, "WRONG")):
        with pytest.raises(Stage4BError):
            execution._execute_openai_development_transaction_v0_2(
                authorization_path=paths[0],
                pricing_path=paths[1],
                data_controls_path=paths[2],
                repository_root=root,
...
def test_cache_miss_orders_marker_before_client_call_cache_and_parse(
    ...
    def provider(_client: object, request: LLMExtractionRequestV02):
        nonlocal provider_calls
        events.append("provider")
        provider_calls += 1
        return _observation(request)

    def local(request: LLMExtractionRequestV02, response: LLMProviderResponse):
        identity = CacheIdentityV02.from_request(request)
...
def test_attempt_marker_without_cache_permanently_blocks_retry(tmp_path: Path) -> None:
    ...
    execution._write_exclusive(
        marker_path, execution.attempt_marker_bytes_v0_2(marker), marker=True
    )
    calls = 0
    with pytest.raises(Stage4BError) as captured:
        _execute(
    ...
    assert calls == 0
    assert load_development_attempt_marker_v0_2(marker_path) == marker
...
def test_losing_exclusive_marker_install_creates_no_failure_record(
    ...
    pending_markers: list[
        execution.OpenAIDevelopmentInvocationAttemptMarkerV02
    ] = []
    counters = {"key": 0, "client": 0, "provider": 0}
    ...
    def racing_write_exclusive(path: Path, payload: bytes, *, marker: bool) -> None:
        if marker and path == marker_path:
            ...
            assert payload == execution.attempt_marker_bytes_v0_2(
                pending_markers[0]
            )
            original_write_exclusive(
                marker_path,
                execution.attempt_marker_bytes_v0_2(competing_marker),
                marker=True,
            )
    ...
    assert counters == {"key": 0, "client": 0, "provider": 0}
    assert load_development_attempt_marker_v0_2(marker_path) == competing_marker
...
def test_canonical_conflicting_marker_fails_before_provider_boundaries(
    ...
    provisional = execution.OpenAIDevelopmentInvocationAttemptMarkerV02.model_construct(
        **values,
        marker_sha256="0" * 64,
    )
    conflicting = (
        execution.OpenAIDevelopmentInvocationAttemptMarkerV02.model_validate(
            {
                **values,
    ...
    execution._write_exclusive(
        marker_path,
        execution.attempt_marker_bytes_v0_2(conflicting),
        marker=True,
    )
    counters = {"key": 0, "client": 0, "provider": 0}
...
def test_client_construction_failure_preserves_marker_and_creates_no_cache(
    ...
    assert load_development_attempt_marker_v0_2(marker_path)
    assert load_development_failure_record_v0_2(failure_path).failure_stage == (
        "client_construction"
    )
    with pytest.raises(Stage4BError) as miss:
        ResponseCache(readiness.cache_root).read(invocation.cache_identity)
...
def test_local_parse_failure_preserves_cache_and_reprocesses_without_calls(
    ...
    def fails_last(request: LLMExtractionRequestV02, response: LLMProviderResponse):
...
def test_first_local_semantic_failure_stops_before_invocation_two(
    ...
    failure = load_development_failure_record_v0_2(first_failure)
...
def test_cache_recovery_preserves_exact_original_same_call_provenance(
    ...
    def observation(_client: object, request: LLMExtractionRequestV02):
        return _observation(
            request,
    ...
    def fail_last(request: LLMExtractionRequestV02, response: LLMProviderResponse):
        nonlocal parse_count
        parse_count += 1
        if parse_count == 8:
            raise Stage4BError(Stage4BErrorCode.SCHEMA_INVALID, "fictional")
...
def test_utc_date_rollover_before_first_miss_stops_before_marker_and_provider(
    ...
    def recovery_provider(_client: object, request: LLMExtractionRequestV02):
...
def test_failure_record_and_raised_error_scrub_credential_fragments(
    ...
    def failure(_client: object, _request: LLMExtractionRequestV02):
    ...
    record = load_development_failure_record_v0_2(failure_path)
...
def test_eight_valid_invocations_install_one_canonical_final_record(
    ...
    assert raw == execution_record_bytes_v0_2(record)
    assert raw.endswith(b"\n") and not raw.endswith(b"\n\n")
    assert load_development_execution_record_v0_2(result.execution_record_path) == record
    with pytest.raises(ValidationError):
    ...
    first_marker = load_development_attempt_marker_v0_2(first_marker_path)
    with pytest.raises(ValidationError):
...
def test_recomputed_final_hash_cannot_hide_contradictory_version_provenance(
    ...
    with pytest.raises(Stage4BError) as captured:
        load_development_execution_record_v0_2(result.execution_record_path)
...
def test_public_execution_api_exposes_no_root_provider_or_cache_bypass() -> None:
    ...
    assert tuple(
        inspect.signature(execution.execute_openai_development_v0_2).parameters
    ) == (
        "authorization_path",
        "pricing_path",
        "data_controls_path",
...
def test_production_provider_observation_annotations_resolve() -> None:
    ...
    assert hints["request"] is LLMExtractionRequestV02


def test_v0_1_and_v0_2_authorizations_cannot_cross() -> None:
    v0_2 = _authorization()
    with pytest.raises(ValidationError):
    ...
    values = {
        **v0_2.model_dump(mode="python", exclude={"authorization_sha256"}),
    ...
    with pytest.raises(ValidationError):
        OpenAIDevelopmentExecutionAuthorizationV02.model_validate(
            v0_1.model_dump(mode="python")
        )


def test_v0_1_and_v0_2_cache_identities_cannot_cross(tmp_path: Path) -> None:
    _, _, readiness = _readiness(tmp_path)
    v0_2 = readiness.invocations[0].cache_identity
    with pytest.raises(ValidationError):
        CacheIdentity.model_validate(v0_2.model_dump(mode="python"))
    ...
    with pytest.raises(ValidationError):
        CacheIdentityV02.model_validate(v0_1.model_dump(mode="python"))


def test_committed_v0_1_marker_and_failure_remain_valid_but_not_v0_2() -> None:
    ...
    with pytest.raises(ValidationError):
        execution.OpenAIDevelopmentInvocationFailureRecordV02.model_validate(
