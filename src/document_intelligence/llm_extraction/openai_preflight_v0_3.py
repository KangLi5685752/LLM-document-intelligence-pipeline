"""Additive v0.3 contract separating compatibility from semantic diagnostics."""
...
PREFLIGHT_AUTHORIZATION_SCOPE: Literal[
    "single-synthetic-openai-preflight-v0.3"
] = "single-synthetic-openai-preflight-v0.3"
PREFLIGHT_INPUT_CLASSIFICATION = v0_1.PREFLIGHT_INPUT_CLASSIFICATION
PREFLIGHT_REQUEST_ID = "synthetic-preflight-request-v0.3"
PREFLIGHT_EVIDENCE_ID = "synthetic-preflight-evidence-v0.3"
PREFLIGHT_BLOCK_ID = "synthetic-preflight-block-v0.3"
PREFLIGHT_SYNTHETIC_TEXT = v0_1.PREFLIGHT_SYNTHETIC_TEXT
EXPECTED_ABSTENTION_WARNING = "abstained_no_supported_candidate"


class OpenAIPreflightAuthorizationV03(OpenAIPreflightAuthorization):
    """Explicit authorization scoped only to the separate v0.3 call."""

    scope: Literal["single-synthetic-openai-preflight-v0.3"]


def build_synthetic_openai_preflight_request() -> LLMExtractionRequest:
    """Build the distinct v0.3 request with unchanged synthetic semantics."""
...
class OpenAIPreflightSemanticDiagnosticV03(BaseModel):
    ...
    def validate_classification(self) -> OpenAIPreflightSemanticDiagnosticV03:
        expected = (
            self.entity_count == 0
...
def _build_semantic_diagnostic(
...
) -> OpenAIPreflightSemanticDiagnosticV03:
    ...
    return OpenAIPreflightSemanticDiagnosticV03(
...
class OpenAIPreflightPostResponseMetadataV03(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_metadata(self) -> OpenAIPreflightPostResponseMetadataV03:
        for field_name in (
...
class OpenAIPreflightPostResponseFailureV03(Stage4BError):
    """Technical output failure carrying only safe returned-response metadata."""

    def __init__(
        self,
        code: Stage4BErrorCode,
        metadata: OpenAIPreflightPostResponseMetadataV03,
    ) -> None:
        self.safe_metadata = OpenAIPreflightPostResponseMetadataV03.model_validate(
            metadata.model_dump(mode="python")
        )
        super().__init__(code, "OpenAI v0.3 response failed technical validation")
...
class OpenAIPreflightRecordV03(OpenAIPreflightRecordV02):
    """Canonical successful compatibility record for the v0.3 call."""

    preflight_schema_version: Literal["0.3"]
    preflight_id: Literal["openai-gpt-5.4-mini-synthetic-preflight-v0.3"]
    authorization: OpenAIPreflightAuthorizationV03
    compatibility_status: Literal["passed"]
    semantic_diagnostic: OpenAIPreflightSemanticDiagnosticV03

    @model_validator(mode="after")
    def validate_record(self) -> OpenAIPreflightRecordV03:
        for field_name in (
            "request_id",
            "returned_model_identifier",
            "version_provenance_source_response_id",
...
def _build_preflight_record(**values: Any) -> OpenAIPreflightRecordV03:
    provisional = OpenAIPreflightRecordV03.model_construct(
        **values,
    ...
    return OpenAIPreflightRecordV03.model_validate(
        {**values, "preflight_record_sha256": record_hash}
    )


def preflight_record_bytes(record: OpenAIPreflightRecordV03) -> bytes:
    """Return canonical v0.3 success bytes after full validation."""
    validated = OpenAIPreflightRecordV03.model_validate(
...
def _validate_authorization(
    ...
    try:
        return OpenAIPreflightAuthorizationV03.model_validate(
            authorization.model_dump(mode="python")
        )
    except (AttributeError, ValidationError) as error:
        raise Stage4BError(
            Stage4BErrorCode.PREFLIGHT_AUTHORIZATION_INVALID,
            "v0.3 preflight authorization is invalid",
        ) from error
...
def run_openai_synthetic_preflight(
    *,
    ...
    """Run one v0.3 call; semantic variance cannot negate compatibility."""
    ...
    if output_failure_code is not None:
        raise OpenAIPreflightPostResponseFailureV03(
            output_failure_code,
            safe_metadata,
        )
    if validated_output is None:
        raise Stage4BError(
            ...
            "v0.3 output validation returned no result",
        )
