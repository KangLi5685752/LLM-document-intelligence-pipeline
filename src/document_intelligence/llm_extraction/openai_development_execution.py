"""Default-deny transaction for the frozen Stage 4D development execution."""
...
EXECUTION_PLAN_ARTIFACT_BYTES = 12641
EXECUTION_CONFIRMATION = "EXECUTE_BOUNDED_FIVE_SOURCE_OPENAI_DEVELOPMENT_V0_1"
...
CONSERVATIVE_COST_CEILING_USD = Decimal("0.9953895")
...
class OpenAIDevelopmentExecutionAuthorizationV01(BaseModel):
    ...
    def validate_authorization(self) -> OpenAIDevelopmentExecutionAuthorizationV01:
...
def development_authorization_bytes(
    authorization: OpenAIDevelopmentExecutionAuthorizationV01,
) -> bytes:
    validated = OpenAIDevelopmentExecutionAuthorizationV01.model_validate(
        authorization.model_dump(mode="python")
    )
    return _canonical_model_bytes(validated)


class OpenAIDevelopmentInvocationAttemptMarkerV01(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_marker(self) -> OpenAIDevelopmentInvocationAttemptMarkerV01:
        expected_path = (
...
class OpenAIDevelopmentInvocationFailureRecordV01(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_failure(self) -> OpenAIDevelopmentInvocationFailureRecordV01:
...
class OpenAIDevelopmentInvocationOutcomeV01(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_outcome(self) -> OpenAIDevelopmentInvocationOutcomeV01:
        for field_name in (
            "request_id",
...
class OpenAIDevelopmentExecutionRecordV01(BaseModel):
    ...
    ordered_invocation_outcomes: tuple[
        OpenAIDevelopmentInvocationOutcomeV01, ...
    ...
    @model_validator(mode="after")
    def validate_record(self) -> OpenAIDevelopmentExecutionRecordV01:
...
def attempt_marker_bytes(marker: OpenAIDevelopmentInvocationAttemptMarkerV01) -> bytes:
    validated = OpenAIDevelopmentInvocationAttemptMarkerV01.model_validate(
        marker.model_dump(mode="python")
    )
    return _canonical_model_bytes(validated)


def failure_record_bytes(
    record: OpenAIDevelopmentInvocationFailureRecordV01,
) -> bytes:
    validated = OpenAIDevelopmentInvocationFailureRecordV01.model_validate(
        record.model_dump(mode="python")
    )
    return _canonical_model_bytes(validated)


def execution_record_bytes(record: OpenAIDevelopmentExecutionRecordV01) -> bytes:
    validated = OpenAIDevelopmentExecutionRecordV01.model_validate(
        record.model_dump(mode="python")
    )
    return _canonical_model_bytes(validated)
...
class PreparedDevelopmentInvocation:
    plan: DevelopmentExecutionInvocationPlanV01
    manifest_identity: OpenAIDevelopmentInvocationIdentityV01
    request: LLMExtractionRequest
    cache_identity: CacheIdentity


@dataclass(frozen=True)
class OpenAIDevelopmentExecutionInputs:
    authorization: OpenAIDevelopmentExecutionAuthorizationV01
...
@dataclass(frozen=True)
class OpenAIDevelopmentExecutionReadiness:
    plan: OpenAIDevelopmentExecutionPlanV01
    manifest: OpenAIDevelopmentManifestV01
    inputs: OpenAIDevelopmentExecutionInputs
    execution_timestamp_utc: datetime
    repository_root: Path
    invocations: tuple[PreparedDevelopmentInvocation, ...]
    cache_root: Path
    execution_record_path: Path
    existing_execution_record: OpenAIDevelopmentExecutionRecordV01 | None


@dataclass(frozen=True)
class OpenAIDevelopmentExecutionResult:
    readiness: OpenAIDevelopmentExecutionReadiness
    record: OpenAIDevelopmentExecutionRecordV01
    execution_record_path: Path
...
def _load_frozen_plan(repository_root: Path) -> OpenAIDevelopmentExecutionPlanV01:
    ...
    if (
        len(raw) != EXECUTION_PLAN_ARTIFACT_BYTES
        or uppercase_sha256_bytes(raw) != EXECUTION_PLAN_OUTER_SHA256
    ):
        raise Stage4BError(
            Stage4BErrorCode.DEVELOPMENT_EXECUTION_GATE_INVALID,
    ...
    try:
        plan = OpenAIDevelopmentExecutionPlanV01.model_validate(
            _strict_json_object(raw, label="frozen development execution plan")
        )
    except Stage4BError:
        raise
    ...
    if development_execution_plan_bytes(plan) != raw:
        raise Stage4BError(
            Stage4BErrorCode.DEVELOPMENT_EXECUTION_GATE_INVALID,
...
def _load_frozen_manifest(
    ...
    if (
        len(raw) != MANIFEST_ARTIFACT_BYTES
        or uppercase_sha256_bytes(raw) != MANIFEST_CANONICAL_LF_SHA256
    ):
        raise Stage4BError(
            Stage4BErrorCode.DEVELOPMENT_EXECUTION_GATE_INVALID,
    ...
    try:
        manifest = OpenAIDevelopmentManifestV01.model_validate(
            _strict_json_object(raw, label="frozen development manifest")
        )
    except Stage4BError:
        raise
    except ValidationError as error:
    ...
    if (
        ...
        or development_manifest_bytes(manifest) != raw
    ):
        raise Stage4BError(
            Stage4BErrorCode.DEVELOPMENT_EXECUTION_GATE_INVALID,
...
def _validate_inputs(
    *,
    ...
    manifest: OpenAIDevelopmentManifestV01,
    timestamp: datetime,
) -> OpenAIDevelopmentExecutionInputs:
    ...
    return OpenAIDevelopmentExecutionInputs(authorization, pricing, data_controls)
...
def _reconstruct_invocations(
    ...
    preparation = prepare_openai_development_manifest(
    ...
    primary: list[tuple[LLMExtractionRequest, str]] = []
    for route in manifest.source_routes:
        primary.extend(
            (request, route.parsed_document_canonical_sha256)
            for request in manifest_module._partition_primary_requests(
                document=documents[route.source_id],
                policy=manifest.partition_policy,
            )
        )
    repeat, repeated_primary_id = manifest_module._repeat_request(
    ...
    for index, ((request, route_hash), plan_item, manifest_item) in enumerate(
        ...
        identity = manifest_module.build_hash_only_invocation_identity(
            request=request,
...
RequestReconstructor: TypeAlias = Callable[
    [Path, OpenAIDevelopmentExecutionPlanV01, OpenAIDevelopmentManifestV01],
    tuple[PreparedDevelopmentInvocation, ...],
]
...
def load_development_attempt_marker(
    path: Path,
) -> OpenAIDevelopmentInvocationAttemptMarkerV01:
    model = _parse_canonical_artifact(
        path,
        label="development attempt marker",
        model_type=OpenAIDevelopmentInvocationAttemptMarkerV01,
    )
    assert isinstance(model, OpenAIDevelopmentInvocationAttemptMarkerV01)
    return model


def load_development_failure_record(
    path: Path,
) -> OpenAIDevelopmentInvocationFailureRecordV01:
    model = _parse_canonical_artifact(
        path,
        label="development failure record",
        model_type=OpenAIDevelopmentInvocationFailureRecordV01,
        hash_field="failure_record_sha256",
        hash_error_code=Stage4BErrorCode.DEVELOPMENT_FAILURE_RECORD_HASH_MISMATCH,
    )
    assert isinstance(model, OpenAIDevelopmentInvocationFailureRecordV01)
    return model


def load_development_execution_record(
    path: Path,
) -> OpenAIDevelopmentExecutionRecordV01:
    model = _parse_canonical_artifact(
        path,
        label="development execution record",
        model_type=OpenAIDevelopmentExecutionRecordV01,
        hash_field="execution_record_sha256",
        hash_error_code=Stage4BErrorCode.DEVELOPMENT_EXECUTION_RECORD_HASH_MISMATCH,
    )
    assert isinstance(model, OpenAIDevelopmentExecutionRecordV01)
    return model


def _validate_existing_marker(
    marker: OpenAIDevelopmentInvocationAttemptMarkerV01,
    *,
    readiness: OpenAIDevelopmentExecutionReadiness,
    invocation: PreparedDevelopmentInvocation,
) -> OpenAIDevelopmentInvocationAttemptMarkerV01:
    expected = {
...
def _validate_openai_development_execution_readiness(
    *,
...
) -> OpenAIDevelopmentExecutionReadiness:
    ...
    authorization = _load_input_model(
        ...
        model_type=OpenAIDevelopmentExecutionAuthorizationV01,
    ...
    assert isinstance(authorization, OpenAIDevelopmentExecutionAuthorizationV01)
    ...
    existing: OpenAIDevelopmentExecutionRecordV01 | None = None
    if os.path.lexists(final_path):
        existing = load_development_execution_record(final_path)
        _validate_existing_record(existing, authorization=authorization, plan=plan)
    return OpenAIDevelopmentExecutionReadiness(
        plan=plan,
        manifest=manifest,
        inputs=inputs,
...
def validate_openai_development_execution_readiness(
    *,
    authorization_path: Path,
    pricing_path: Path,
    data_controls_path: Path,
    ...
    return _validate_openai_development_execution_readiness(
...
def _build_marker(
    *,
    ...
    provisional = OpenAIDevelopmentInvocationAttemptMarkerV01.model_construct(
        **values, marker_sha256="0" * 64
    )
    return OpenAIDevelopmentInvocationAttemptMarkerV01.model_validate(
...
ProviderObservation: TypeAlias = Callable[
    [object, LLMExtractionRequest], OpenAIPreflightProviderObservation
]
LocalValidator: TypeAlias = Callable[
    [LLMExtractionRequest, LLMProviderResponse], ValidatedCandidateOutput
]
...
def _build_failure_record(
    *,
    readiness: OpenAIDevelopmentExecutionReadiness,
    invocation: PreparedDevelopmentInvocation,
    marker: OpenAIDevelopmentInvocationAttemptMarkerV01 | None,
    timestamp: datetime,
    ...
    provisional = OpenAIDevelopmentInvocationFailureRecordV01.model_construct(
        **values, failure_record_sha256="0" * 64
    )
    return OpenAIDevelopmentInvocationFailureRecordV01.model_validate(
        {
            **values,
...
def _outcome(
    *,
    ...
    return OpenAIDevelopmentInvocationOutcomeV01(
        invocation_order=invocation.plan.invocation_order,
        request_id=invocation.plan.request_id,
        source_id=invocation.plan.source_id,
...
def _build_execution_record(
    *,
    ...
    started: datetime,
    completed: datetime,
    outcomes: tuple[OpenAIDevelopmentInvocationOutcomeV01, ...],
) -> OpenAIDevelopmentExecutionRecordV01:
    values = {
    ...
    provisional = OpenAIDevelopmentExecutionRecordV01.model_construct(
        **values, execution_record_sha256="0" * 64
    )
    return OpenAIDevelopmentExecutionRecordV01.model_validate(
        {
            **values,
...
def _execute_openai_development_transaction(
    *,
    authorization_path: Path,
    pricing_path: Path,
    data_controls_path: Path,
...
) -> OpenAIDevelopmentExecutionResult:
    ...
    readiness = _validate_openai_development_execution_readiness(
        authorization_path=authorization_path,
        pricing_path=pricing_path,
        data_controls_path=data_controls_path,
        repository_root=repository_root,
    ...
    if readiness.existing_execution_record is not None:
        return OpenAIDevelopmentExecutionResult(
            readiness=readiness,
    ...
    outcomes: list[OpenAIDevelopmentInvocationOutcomeV01] = []
    provider_calls = 0
    attempts = 0
    output_tokens = 0
    new_cost = Decimal("0")
    ...
    for invocation in readiness.invocations:
        ...
        marker: OpenAIDevelopmentInvocationAttemptMarkerV01 | None = None
        ...
        try:
            ...
            if cached is None:
                ...
                if os.path.lexists(marker_path):
                    _validate_existing_marker(
                        load_development_attempt_marker(marker_path),
                        readiness=readiness,
                        invocation=invocation,
                    )
                    raise Stage4BError(
                        Stage4BErrorCode.DEVELOPMENT_ATTEMPT_ALREADY_EXISTS,
                        "attempt marker without cache permanently blocks v0.1 retry",
                    )
                if os.path.lexists(failure_path):
                    load_development_failure_record(failure_path)
                    raise Stage4BError(
                        Stage4BErrorCode.DEVELOPMENT_ATTEMPT_ALREADY_EXISTS,
                        "failure artifact without cache blocks v0.1 execution",
                    )
                if (
                ...
                _write_exclusive(
                    marker_path,
                    attempt_marker_bytes(pending_marker),
                    marker=True,
                )
            ...
            if durable_invocation_state:
                ...
                if not os.path.lexists(failure_path):
                    _write_exclusive(
                        failure_path, failure_record_bytes(failure), marker=False
                    )
    ...
    try:
        _write_exclusive(
            ...
            execution_record_bytes(record),
            marker=False,
        )
    except Stage4BError:
        if os.path.lexists(readiness.execution_record_path):
            existing = load_development_execution_record(
                readiness.execution_record_path
            )
            if existing == record:
                record = existing
...
def execute_openai_development(
    *,
    authorization_path: Path,
    pricing_path: Path,
    data_controls_path: Path,
    ...
    root = resolve_production_repository_root(Path.cwd())
    return _execute_openai_development_transaction(
        authorization_path=authorization_path,
        pricing_path=pricing_path,
        data_controls_path=data_controls_path,
        repository_root=root,
...
__all__ = [
    ...
    "validate_openai_development_execution_readiness",
]
