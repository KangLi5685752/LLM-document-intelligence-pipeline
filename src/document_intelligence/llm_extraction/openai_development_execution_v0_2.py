"""Default-deny transaction for the frozen Stage 4D v0.2 execution."""
...
from document_intelligence.llm_extraction import (
    openai_preflight_execution_v0_2 as preflight_v0_2_execution,
)
...
EXECUTION_PLAN_ARTIFACT_BYTES = 13064
EXECUTION_CONFIRMATION = "EXECUTE_BOUNDED_FIVE_SOURCE_OPENAI_DEVELOPMENT_V0_2"
...
CONSERVATIVE_COST_CEILING_USD = Decimal("1.000869")
...
class OpenAIDevelopmentExecutionAuthorizationV02(BaseModel):
    ...
    def validate_authorization(self) -> OpenAIDevelopmentExecutionAuthorizationV02:
...
def development_authorization_bytes_v0_2(
    authorization: OpenAIDevelopmentExecutionAuthorizationV02,
) -> bytes:
    validated = OpenAIDevelopmentExecutionAuthorizationV02.model_validate(
        authorization.model_dump(mode="python")
    )
    return _canonical_model_bytes(validated)


class OpenAIDevelopmentInvocationAttemptMarkerV02(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_marker(self) -> OpenAIDevelopmentInvocationAttemptMarkerV02:
        expected_path = (
...
class OpenAIDevelopmentInvocationFailureRecordV02(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_failure(self) -> OpenAIDevelopmentInvocationFailureRecordV02:
...
class OpenAIDevelopmentInvocationOutcomeV02(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_outcome(self) -> OpenAIDevelopmentInvocationOutcomeV02:
        for field_name in (
            "request_id",
...
class OpenAIDevelopmentExecutionRecordV02(BaseModel):
    ...
    ordered_invocation_outcomes: tuple[
        OpenAIDevelopmentInvocationOutcomeV02, ...
    ...
    @model_validator(mode="after")
    def validate_record(self) -> OpenAIDevelopmentExecutionRecordV02:
...
def attempt_marker_bytes_v0_2(marker: OpenAIDevelopmentInvocationAttemptMarkerV02) -> bytes:
    validated = OpenAIDevelopmentInvocationAttemptMarkerV02.model_validate(
        marker.model_dump(mode="python")
    )
    return _canonical_model_bytes(validated)


def failure_record_bytes_v0_2(
    record: OpenAIDevelopmentInvocationFailureRecordV02,
) -> bytes:
    validated = OpenAIDevelopmentInvocationFailureRecordV02.model_validate(
        record.model_dump(mode="python")
    )
    return _canonical_model_bytes(validated)


def execution_record_bytes_v0_2(record: OpenAIDevelopmentExecutionRecordV02) -> bytes:
    validated = OpenAIDevelopmentExecutionRecordV02.model_validate(
        record.model_dump(mode="python")
    )
    return _canonical_model_bytes(validated)
...
class PreparedDevelopmentInvocationV02:
    plan: DevelopmentExecutionInvocationPlanV02
    manifest_identity: OpenAIDevelopmentInvocationIdentityV02
    request: LLMExtractionRequestV02
    cache_identity: CacheIdentityV02


@dataclass(frozen=True)
class OpenAIDevelopmentExecutionInputsV02:
    authorization: OpenAIDevelopmentExecutionAuthorizationV02
...
@dataclass(frozen=True)
class OpenAIDevelopmentExecutionReadinessV02:
    plan: OpenAIDevelopmentExecutionPlanV02
    manifest: OpenAIDevelopmentManifestV02
    inputs: OpenAIDevelopmentExecutionInputsV02
    execution_timestamp_utc: datetime
    repository_root: Path
    invocations: tuple[PreparedDevelopmentInvocationV02, ...]
    cache_root: Path
    execution_record_path: Path
    existing_execution_record: OpenAIDevelopmentExecutionRecordV02 | None


@dataclass(frozen=True)
class OpenAIDevelopmentExecutionResultV02:
    readiness: OpenAIDevelopmentExecutionReadinessV02
    record: OpenAIDevelopmentExecutionRecordV02
    execution_record_path: Path
...
def _load_frozen_plan(repository_root: Path) -> OpenAIDevelopmentExecutionPlanV02:
    ...
    try:
        plan = OpenAIDevelopmentExecutionPlanV02.model_validate(
            _strict_json_object(
    ...
    if development_execution_plan_bytes_v0_2(plan) != canonical_raw:
        raise Stage4BError(
            Stage4BErrorCode.DEVELOPMENT_EXECUTION_GATE_INVALID,
...
def _load_frozen_manifest(
    ...
    try:
        manifest = OpenAIDevelopmentManifestV02.model_validate(
    ...
    if (
        ...
        or development_manifest_bytes_v0_2(manifest) != canonical_raw
    ):
        raise Stage4BError(
            Stage4BErrorCode.DEVELOPMENT_EXECUTION_GATE_INVALID,
...
def _validate_inputs(
    *,
    ...
    return OpenAIDevelopmentExecutionInputsV02(authorization, pricing, data_controls)
...
def _reconstruct_invocations(
    ...
    plan: OpenAIDevelopmentExecutionPlanV02,
    manifest: OpenAIDevelopmentManifestV02,
) -> tuple[PreparedDevelopmentInvocationV02, ...]:
    ...
    preparation = prepare_openai_development_manifest_v0_2(
    ...
    primary: list[tuple[LLMExtractionRequestV02, str]] = []
    for route in manifest.source_routes:
        primary.extend(
            (request, route.parsed_document_canonical_sha256)
            for request in manifest_module._partition_primary_requests_v0_2(
                document=documents[route.source_id],
                policy=manifest.partition_policy,
            )
        )
    repeat, repeated_primary_id = manifest_module._repeat_request_v0_2(
    ...
    prepared: list[PreparedDevelopmentInvocationV02] = []
    for index, ((request, route_hash), plan_item, manifest_item) in enumerate(
        ...
        identity = manifest_module.build_hash_only_invocation_identity_v0_2(
            request=request,
        ...
        cache_identity = CacheIdentityV02.from_request(request)
        payload = canonical_json_bytes(build_openai_responses_payload(request))
        ...
        prepared.append(
            PreparedDevelopmentInvocationV02(
                plan=plan_item,
...
RequestReconstructor: TypeAlias = Callable[
    [Path, OpenAIDevelopmentExecutionPlanV02, OpenAIDevelopmentManifestV02],
    tuple[PreparedDevelopmentInvocationV02, ...],
]
...
def load_development_attempt_marker_v0_2(
    path: Path,
) -> OpenAIDevelopmentInvocationAttemptMarkerV02:
    model = _parse_canonical_artifact(
        path,
        label="development attempt marker",
        model_type=OpenAIDevelopmentInvocationAttemptMarkerV02,
    )
    assert isinstance(model, OpenAIDevelopmentInvocationAttemptMarkerV02)
    return model


def load_development_failure_record_v0_2(
    path: Path,
) -> OpenAIDevelopmentInvocationFailureRecordV02:
    model = _parse_canonical_artifact(
        path,
        label="development failure record",
        model_type=OpenAIDevelopmentInvocationFailureRecordV02,
        hash_field="failure_record_sha256",
        hash_error_code=Stage4BErrorCode.DEVELOPMENT_FAILURE_RECORD_HASH_MISMATCH,
    )
    assert isinstance(model, OpenAIDevelopmentInvocationFailureRecordV02)
    return model


def load_development_execution_record_v0_2(
    path: Path,
) -> OpenAIDevelopmentExecutionRecordV02:
    model = _parse_canonical_artifact(
        path,
        label="development execution record",
        model_type=OpenAIDevelopmentExecutionRecordV02,
        hash_field="execution_record_sha256",
        hash_error_code=Stage4BErrorCode.DEVELOPMENT_EXECUTION_RECORD_HASH_MISMATCH,
    )
    assert isinstance(model, OpenAIDevelopmentExecutionRecordV02)
    return model


def _validate_existing_marker(
    marker: OpenAIDevelopmentInvocationAttemptMarkerV02,
    *,
    readiness: OpenAIDevelopmentExecutionReadinessV02,
    invocation: PreparedDevelopmentInvocationV02,
) -> OpenAIDevelopmentInvocationAttemptMarkerV02:
    expected = {
...
def _validate_openai_development_execution_readiness_v0_2(
    *,
...
) -> OpenAIDevelopmentExecutionReadinessV02:
    ...
    authorization = _load_input_model(
        ...
        model_type=OpenAIDevelopmentExecutionAuthorizationV02,
    ...
    assert isinstance(authorization, OpenAIDevelopmentExecutionAuthorizationV02)
    ...
    existing: OpenAIDevelopmentExecutionRecordV02 | None = None
    if os.path.lexists(final_path):
        existing = load_development_execution_record_v0_2(final_path)
        _validate_existing_record(existing, authorization=authorization, plan=plan)
    return OpenAIDevelopmentExecutionReadinessV02(
        plan=plan,
        manifest=manifest,
        inputs=inputs,
...
def validate_openai_development_execution_readiness_v0_2(
    *,
    authorization_path: Path,
    pricing_path: Path,
    data_controls_path: Path,
    ...
    return _validate_openai_development_execution_readiness_v0_2(
...
def _build_marker(
    *,
    ...
    provisional = OpenAIDevelopmentInvocationAttemptMarkerV02.model_construct(
        **values, marker_sha256="0" * 64
    )
    return OpenAIDevelopmentInvocationAttemptMarkerV02.model_validate(
...
def _production_provider_observation(
    ...
    return OpenAIResponsesPreflightBridge(
        provider=OpenAIResponsesProvider(client=client)  # type: ignore[arg-type]
    ).generate_preflight(request)


ProviderObservation: TypeAlias = Callable[
    [object, LLMExtractionRequestV02], OpenAIPreflightProviderObservation
]
LocalValidator: TypeAlias = Callable[
    [LLMExtractionRequestV02, LLMProviderResponse], ValidatedCandidateOutput
]
...
def _sanitized_failure(
    ...
    if isinstance(error, OpenAIProviderFailure):
        diagnostics = (
            preflight_v0_2_execution._credential_scrubbed_diagnostics(
...
def _build_failure_record(
    *,
    readiness: OpenAIDevelopmentExecutionReadinessV02,
    invocation: PreparedDevelopmentInvocationV02,
    marker: OpenAIDevelopmentInvocationAttemptMarkerV02 | None,
    timestamp: datetime,
    ...
    provisional = OpenAIDevelopmentInvocationFailureRecordV02.model_construct(
        **values, failure_record_sha256="0" * 64
    )
    return OpenAIDevelopmentInvocationFailureRecordV02.model_validate(
        {
            **values,
...
def _outcome(
    *,
    ...
    return OpenAIDevelopmentInvocationOutcomeV02(
        invocation_order=invocation.plan.invocation_order,
        request_id=invocation.plan.request_id,
        source_id=invocation.plan.source_id,
...
def _build_execution_record(
    *,
    ...
    started: datetime,
    completed: datetime,
    outcomes: tuple[OpenAIDevelopmentInvocationOutcomeV02, ...],
) -> OpenAIDevelopmentExecutionRecordV02:
    values = {
    ...
    provisional = OpenAIDevelopmentExecutionRecordV02.model_construct(
        **values, execution_record_sha256="0" * 64
    )
    return OpenAIDevelopmentExecutionRecordV02.model_validate(
        {
            **values,
...
def _execute_openai_development_transaction_v0_2(
    *,
    authorization_path: Path,
    pricing_path: Path,
    data_controls_path: Path,
...
) -> OpenAIDevelopmentExecutionResultV02:
    ...
    readiness = _validate_openai_development_execution_readiness_v0_2(
        authorization_path=authorization_path,
        pricing_path=pricing_path,
        data_controls_path=data_controls_path,
        repository_root=repository_root,
    ...
    if readiness.existing_execution_record is not None:
        return OpenAIDevelopmentExecutionResultV02(
            readiness=readiness,
    ...
    outcomes: list[OpenAIDevelopmentInvocationOutcomeV02] = []
    provider_calls = 0
    attempts = 0
    output_tokens = 0
    new_cost = Decimal("0")
    ...
    for invocation in readiness.invocations:
        ...
        marker: OpenAIDevelopmentInvocationAttemptMarkerV02 | None = None
        ...
        try:
            ...
            if cached is None:
                ...
                if os.path.lexists(marker_path):
                    _validate_existing_marker(
                        load_development_attempt_marker_v0_2(marker_path),
                        readiness=readiness,
                        invocation=invocation,
                    )
                    raise Stage4BError(
                        Stage4BErrorCode.DEVELOPMENT_ATTEMPT_ALREADY_EXISTS,
                        "attempt marker without cache permanently blocks v0.2 retry",
                    )
                if os.path.lexists(failure_path):
                    load_development_failure_record_v0_2(failure_path)
                    raise Stage4BError(
                        Stage4BErrorCode.DEVELOPMENT_ATTEMPT_ALREADY_EXISTS,
                        "failure artifact without cache blocks v0.2 execution",
                    )
                if (
                ...
                _write_exclusive(
                    marker_path,
                    attempt_marker_bytes_v0_2(pending_marker),
                    marker=True,
                )
                ...
                credential = preflight_v0_2_execution.validate_openai_api_key_shape(
                    supplied
                )
            ...
            if durable_invocation_state:
                ...
                if not os.path.lexists(failure_path):
                    _write_exclusive(
                        failure_path, failure_record_bytes_v0_2(failure), marker=False
                    )
    ...
    try:
        _write_exclusive(
            ...
            execution_record_bytes_v0_2(record),
            marker=False,
        )
    except Stage4BError:
        if os.path.lexists(readiness.execution_record_path):
            existing = load_development_execution_record_v0_2(
                readiness.execution_record_path
            )
            if existing == record:
                record = existing
...
def execute_openai_development_v0_2(
    *,
    authorization_path: Path,
    pricing_path: Path,
    data_controls_path: Path,
    ...
    root = resolve_production_repository_root(Path.cwd())
    return _execute_openai_development_transaction_v0_2(
        authorization_path=authorization_path,
        pricing_path=pricing_path,
        data_controls_path=data_controls_path,
        repository_root=root,
...
__all__ = [
    ...
    "OpenAIDevelopmentExecutionAuthorizationV02",
    "OpenAIDevelopmentExecutionReadinessV02",
    "OpenAIDevelopmentExecutionRecordV02",
    "OpenAIDevelopmentExecutionResultV02",
    "OpenAIDevelopmentInvocationAttemptMarkerV02",
    ...
    "OpenAIDevelopmentInvocationOutcomeV02",
    "PreparedDevelopmentInvocationV02",
    "attempt_marker_bytes_v0_2",
    "development_authorization_bytes_v0_2",
    "execute_openai_development_v0_2",
    "execution_record_bytes_v0_2",
    "failure_record_bytes_v0_2",
    "load_development_attempt_marker_v0_2",
    "load_development_execution_record_v0_2",
    "load_development_failure_record_v0_2",
    "validate_openai_development_execution_readiness_v0_2",
]
