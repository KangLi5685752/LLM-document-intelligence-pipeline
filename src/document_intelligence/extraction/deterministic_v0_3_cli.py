"""Single-document CLI for deterministic-baseline-v0.3 extraction."""
...
def build_parser() -> argparse.ArgumentParser:
    """Build the bounded single-document v0.3 command-line interface."""

    parser = argparse.ArgumentParser(
        description=(
            "Extract one ParsedDocument with deterministic-baseline-v0.3; "
...
def main(argv: list[str] | None = None) -> int:
    """Validate one input, run v0.3, and emit canonical JSON."""
    ...
    try:
        ...
        rendered = canonical_candidate_result_json_v0_3(
            extract_deterministic_candidates_v0_3(document)
        )
