"""Strict additive evaluation contracts for deterministic-baseline-v0.2."""
...
def _require_source_mapping(values: dict[str, int], label: str) -> None:
    if tuple(values) != DEVELOPMENT_SOURCE_IDS:
        raise ValueError(f"{label} must use the frozen development source order")
    if any(value < 0 for value in values.values()):
        raise ValueError(f"{label} must contain non-negative counts")


class MetricFraction(BaseModel):
    """An exact numerator, denominator, and derived value."""
...
class DevelopmentExtractionAttempt(BaseModel):
    """One explicit successful or failed development-source attempt."""
    ...
    canonical_output_sha256: str | None = Field(default=None, pattern=SHA256_PATTERN)
    ...
    def validate_attempt(self) -> DevelopmentExtractionAttempt:
        ...
        if self.result is not None:
            try:
                validated = CandidateExtractionResult.model_validate(
                    self.result.model_dump()
                )
            except ValidationError as error:
                raise ValueError("result must satisfy candidate schema 0.1") from error
            if validated != self.result:
                raise ValueError("result must satisfy candidate schema 0.1")
            ...
            if not SNAKE_CASE_PATTERN.fullmatch(self.error_code):
                raise ValueError("error_code must use lowercase snake_case")
        return self


class ReproducibilityCheck(BaseModel):
    """Source-level comparison of primary and repeat canonical outputs."""
...
class StrictFactMatch(BaseModel):
    """Compact provenance-only record for one strict pairing."""
    ...
    def validate_match(self) -> StrictFactMatch:
        _require_sorted_unique(
            self.qualifier_over_specification, "qualifier_over_specification"
        )
        return self
...
class ChallengeCaseAssessment(BaseModel):
    """Explicit, non-inferred owner outcome for one development challenge."""
...
class PreliminaryDevelopmentEvaluationReport(BaseModel):
    """Deterministic first-observation diagnostics, including incomplete runs."""

    model_config = ConfigDict(extra="forbid", frozen=True, strict=True)

    report_schema_version: Literal["0.1"] = "0.1"
    experiment_id: Literal["deterministic-baseline-v0.2"] = EXPERIMENT_ID
    report_status: Literal["preliminary_incomplete_or_unreviewed"] = (
        "preliminary_incomplete_or_unreviewed"
    )
    ...
    @model_validator(mode="after")
    def validate_report(self) -> PreliminaryDevelopmentEvaluationReport:
        if self.source_ids != DEVELOPMENT_SOURCE_IDS:
            raise ValueError("source_ids must match the frozen development order")
        if self.attempted_source_count != len(DEVELOPMENT_SOURCE_IDS):
        ...
        _require_source_mapping(
            self.candidate_counts_by_source, "candidate_counts_by_source"
        )
        if tuple(self.candidate_counts_by_predicate) != tuple(
            sorted(self.candidate_counts_by_predicate)
        ):
            raise ValueError("candidate_counts_by_predicate must be sorted")
        if any(value < 0 for value in self.candidate_counts_by_predicate.values()):
            raise ValueError("candidate predicate counts must be non-negative")
        ...
        if sum(self.candidate_counts_by_predicate.values()) != self.total_candidate_count:
            raise ValueError("candidate predicate counts must reconcile")
        ...
        allowed_sources = set(DEVELOPMENT_SOURCE_IDS)
        if any(item.source_id not in allowed_sources for item in self.strict_matches):
            raise ValueError("strict_matches contain a non-development source")
        if any(item.source_id not in allowed_sources for item in self.value_alignments):
            raise ValueError("value_alignments contain a non-development source")
        strict_candidate_ids = tuple(item.candidate_id for item in self.strict_matches)
        strict_annotation_ids = tuple(item.annotation_id for item in self.strict_matches)
        alignment_candidate_ids = tuple(
            item.candidate_id for item in self.value_alignments
        )
        alignment_annotation_ids = tuple(
            item.annotation_id for item in self.value_alignments
        )
        for values, label in (
            (strict_candidate_ids, "strict match candidate IDs"),
            (strict_annotation_ids, "strict match annotation IDs"),
            (alignment_candidate_ids, "value alignment candidate IDs"),
            (alignment_annotation_ids, "value alignment annotation IDs"),
        ):
            if len(values) != len(set(values)):
                raise ValueError(f"{label} must be unique")
        _require_sorted_unique(self.unmatched_candidate_ids, "unmatched_candidate_ids")
        _require_sorted_unique(
            self.unmatched_annotation_ids, "unmatched_annotation_ids"
        )
        _require_sorted_unique(self.warnings, "warnings")
        if not set(self.warnings).issubset(EVALUATOR_WARNING_CODES):
        ...
        if self.strict_matches != expected_match_order:
            raise ValueError("strict_matches must use deterministic order")
        expected_alignment_order = tuple(
            sorted(
                self.value_alignments,
                key=lambda item: (item.source_id, item.candidate_id, item.annotation_id),
            )
        )
        if self.value_alignments != expected_alignment_order:
        ...
        if len(self.reproducibility_checks) != 5 or tuple(
            item.source_id for item in self.reproducibility_checks
        ) != self.source_ids:
        ...
        expected_fractions = {
            ...
            "schema_valid_result_rate": MetricFraction.from_counts(
                self.schema_valid_source_count, self.attempted_source_count
            ),
...
class DevelopmentEvaluationReport(PreliminaryDevelopmentEvaluationReport):
    """Complete owner-reviewed deterministic-baseline-v0.2 evaluation."""
    ...
    @model_validator(mode="after")
    def validate_complete_report(self) -> DevelopmentEvaluationReport:
        if self.schema_valid_source_count != 5 or self.failed_source_count != 0:
            raise ValueError("complete evaluation requires five successful sources")
        if not self.all_outputs_byte_identical:
            raise ValueError("complete evaluation requires byte-identical repeats")
        ...
        if case_ids != DEVELOPMENT_CASE_IDS:
            raise ValueError(
                "challenge assessment case IDs must match frozen development cases"
            )
        expected = MetricFraction.from_counts(
            sum(item.outcome == "passed" for item in self.challenge_case_assessments),
            3,
        )
        if self.development_challenge_case_pass_rate != expected:
            raise ValueError(
                "development_challenge_case_pass_rate does not reconcile"
            )
        return self


PreliminaryObservationReport = PreliminaryDevelopmentEvaluationReport
