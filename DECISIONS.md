...
## DEC-100: Select OpenAI Responses API and GPT-5.4 mini for the first LLM comparator

- **Context:** `llm-extraction-baseline-v0.1` requires one provider and requested model before a real adapter or controlled development manifest can be prepared. Official information reviewed on 2026-08-03 documented GPT-5.4 mini with a 400,000-token context window and observed list pricing of USD 0.75 per million input tokens and USD 4.50 per million output tokens; availability and pricing remain subject to preflight verification.
- **Alternatives:** Use multiple providers; select the smallest available model; rely on free-tier execution; use a local open-weight model; select OpenAI Responses API with `gpt-5.4-mini` for the first comparator.
- **Chosen option:** Select OpenAI as the sole provider, the Responses API as the API surface and `gpt-5.4-mini` as the requested model alias for the development-only comparator. This decision does not authorize an API call or claim that the alias is an exact reproducibility identity.
- **Reason:** The selection provides a bounded, relatively inexpensive comparator that can use strict structured output while keeping attribution, cost and integration scope narrow.
- **Trade-off:** The experiment will not compare providers, model access must still be verified for the project account, and the exact returned model identity and current pricing must be captured before execution.

## DEC-101: Require strict text-only structured output with no tools

- **Context:** The comparator needs evidence-linked candidate JSON, not agentic browsing, retrieval or file processing. Additional tools or execution modes would widen the data boundary and complicate provenance.
- **Alternatives:** Permit provider tools and retrieval; accept unconstrained text and repair it locally; require text-only strict JSON Schema Structured Outputs with all tools disabled.
- **Chosen option:** Use text-only input and strict JSON Schema Structured Outputs. Require `store=false`; send no tools; disable web search, file search, function calling, background mode and streaming for the controlled first execution; and prohibit browsing, retrieval and file upload.
- **Reason:** A narrow request shape aligns with the existing provider-neutral contracts, supports fail-closed validation and keeps transmitted content limited to approved development blocks.
- **Trade-off:** The adapter cannot use provider tools or background execution, streaming is unavailable for the first controlled run, and `store=false` must not be represented as a zero-retention guarantee.

## DEC-102: Block manifest freeze until exact model identity and current pricing are verified

- **Context:** A mutable model alias and dated pricing observation do not establish a reproducible execution identity or current cost basis. Exact API compatibility and account availability also remain unverified.
- **Alternatives:** Freeze a manifest using only the requested alias and dated pricing; make an exploratory request without a recorded identity; require a separately authorized preflight before manifest freeze.
- **Chosen option:** Block real manifest freeze until a later preflight verifies account access and strict structured-output compatibility, records the exact model identifier returned by the API, every provider-exposed model-version or snapshot identifier, provider request ID and official SDK version, and rechecks current token pricing. When no separate snapshot/version identifier is exposed, the provenance field must contain the literal value `unavailable`; absence must not be silently omitted or inferred. Fail closed if a stable auditable identity cannot be established.
- **Reason:** The preflight gate prevents a convenient alias or stale price from being mistaken for complete provenance and keeps execution terms fixed before observation.
- **Trade-off:** Adapter work, preflight authorization and manifest review add a separate checkpoint before the bounded development run can begin.
